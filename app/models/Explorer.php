...
class Explorer extends Model
{
    ...
    public function projects($search_query = "")
    {
        ...
        $query = "SELECT COUNT(p.id) as ttl, p
        .project_category FROM projects p LEFT JOIN mdas m ON p.mda_id = m.id
        LEFT JOIN contractors ct ON p.id = ct.project_id LEFT JOIN tender t ON p.id = t.project_id  WHERE p.published = 'yes' " . $attach . " GROUP BY project_category";
        $result = $this->query($query);

        
        $categories = [];
        while ($row = mysqli_fetch_array($result)) {
            $categories[$row['project_category']] = $row['ttl'];
        }
    ...
    public function indexData($search_query = ''){
        ...
        $query = "SELECT SQL_CALC_FOUND_ROWS DISTINCT p.oc_id,p.mda_id,p.title,p.year, p.id, p.state, p.status, p.monitored, p.description, p.year, m.short_name, m.name AS mda, b.budget_amount, c.amount, ct.contractor_id, i.name FROM projects p LEFT JOIN mdas m
        ON p.mda_id = m.id LEFT JOIN planning b on p.id = b.project_id LEFT JOIN contract c ON c.project_id = p.id LEFT JOIN contractors ct ON p.id = ct.project_id LEFT JOIN institutions i ON ct.contractor_id = i.id WHERE p.published = 'yes' AND 
         p.monitored = 'yes'";

        $results = $this->query($query);
        if (mysqli_num_rows($results) <= 0) {
            $this->verifiedTable = "<td> No Verified Projects </td>";
        } else {
            
            $this->verifiedTable = "";
            while ($row = mysqli_fetch_array($results)) {

                $budgetAmount = empty((int)$row["budget_amount"]) ? "Not Provided" : number_format($row["budget_amount"], 2);
                $contractAmount = empty((int)$row["amount"]) ? "Not Provided" : number_format($row["amount"], 2);
                $this->verifiedTable .= $this->renderCompareTable($row["id"], $row["title"], $row["state"], ucwords(strtolower($row["name"])), $budgetAmount, $contractAmount, $row["year"], $row["mda"] . "  (" . strtoupper($row["short_name"]) . ")", $row["status"]);
            }
